...
| Task 2 notebook | The first fix wrapped `merge_and_unload()` in `try/except MemoryError` and fell back on failure. It crashed again at the identical point, fallback never reached: Linux's OOM-killer **SIGKILLs** the process, so no Python exception is raised and no handler can run. Recovery after the fact is not possible | Size the base model via `HfApi(files_metadata=True)` against `psutil.virtual_memory().available` and choose the merge path **before** allocating anything; the cell is also runnable on a fresh runtime so a Restart can be used to reclaim training memory |
