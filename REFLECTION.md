...
reason over indicator *combinations* rather than echo values. The `TradingSignal` validator rejects a justification unless it spans two named indicators and
...
**Structure over instruction (Task 3).** I chose LangGraph over CrewAI because three criteria
are about enforcement, not claims. Agent B cannot call the price tools
because the tool objects are never placed in its node's list — a system prompt saying "don't"
is a request; not passing the tool is a guarantee. The observe-replan cycle isn't logged; it *is* the `agent → tools → agent` edge. More code than CrewAI; far easier to defend.
...
the obvious choice. On a mode-collapsed corpus it scored 0.41 and
...
The second surfaced in a live run. Task 3 died on a provider 400,
`tool_use_failed`: the model copied fifteen headlines verbatim into `llm_sentiment`'s
...
removed the failure class, and closed a hole: a model copying text can
